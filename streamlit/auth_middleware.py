...
def logout():
    """Faz logout removendo o cookie de autenticação e limpando estados da sessão"""
    # Remove o cookie de autenticação
    if "auth_cookie" in st.session_state:
        del st.session_state["auth_cookie"]

    # Reset todos os estados relevantes ao login
    st.session_state["password_correct"] = False
    st.session_state["login_attempt"] = False

    # Limpa também os parâmetros de autenticação da URL se existirem
    if "auth" in st.query_params:
        st.query_params.clear()

    # Limpa todos os outros estados relacionados à sessão do chat
    for key in list(st.session_state.keys()):
        if key in ["messages", "chat_history", "session_id", "current_chat_index",
                   "chat_title", "user_input", "direct_text"]:
